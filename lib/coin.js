...
function getJob(miner, activeBlockTemplate, bashCache) {
    ...
    if (typeof (activeBlockTemplate.seed_hash) !== 'undefined') {
        miner.cachedJob.seed_hash = activeBlockTemplate.seed_hash;
    }
    if (typeof (activeBlockTemplate.next_seed_hash) !== 'undefined') {
        miner.cachedJob.next_seed_hash = activeBlockTemplate.next_seed_hash;
    }
...
function getMasterJob(pool, workerID) {
    ...
    let workerData = {
        ...
        next_seed_hash: activeBlockTemplate.next_seed_hash
    };
    let localData = {
        id: workerData.id,
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (++ verified_share_num <= MAX_VER_SHARES_PER_SEC*VER_SHARES_PERIOD) {
            ...
            hash = (typeof blockTemplate.seed_hash !== 'undefined' && blockTemplate.seed_hash) ?
                multiHashing[miner.coinSettings.algo](convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), miner.coinSettings.variant) :
                (typeof miner.coinSettings.includeHeight !== 'undefined' && miner.coinSettings.includeHeight) ?
